...
function help()
{
	echo "Usage: server-daemon.sh (start|stop) admin server"
}
...
case $action in
	...
	JAVA_OPT="-server -Xms512m -Xmx512m -XX:NewSize=256m -XX:MaxNewSize=256m -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=70 -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintTenuringDistribution -Xloggc:$SERVER_LOGS/gc.log -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=$SERVER_LOGS/gc_dump"
    nohup java $JAVA_OPT -DSERVER_HOME=$SERVER_HOME -Djava.library.path=$JAVA_LIBRARY_PATH  -classpath $CLASSPATH com.j13.alistar.starter.ServerStarter $SERVER_TMP >>$SERVER_LOGS_OUT_FILE 2>&1 &
